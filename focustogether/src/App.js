...
const DISPLAY_NAME = MY_NAME;
const PARTNER_DISPLAY_NAME = "Baby";
    ...
    >
      <div
        ...
        style={{
          ...
          maxWidth: 360,
          width: "90%",
          boxShadow: "0 0 80px rgba(244,63,94,0.35)",
...
export default function App() {
  ...
  // Use ref to track seen message ID — doesn't cause re-renders, persists across listener updates
  const seenMsgIdRef = useRef(
    typeof localStorage !== "undefined" ? localStorage.getItem(`seenMsg_${MY_ID}`) : null
  );

  const intervalRef  = useRef(null);
  const startTimeRef = useRef(null);  // for accurate timer
  const baseTimeRef  = useRef(0);     // accumulated time before this session start
  const audioCtxRef  = useRef(null);
  const myGoalRef    = useRef(myGoal);

  useEffect(() => { myGoalRef.current = myGoal; }, [myGoal]);
  ...
  // Listen to incoming messages — FIX: use ref for seen ID, store in localStorage
  useEffect(() => {
    const unsub = onValue(ref(db, "messages/" + MY_ID), snap => {
      ...
      if (arr.length > 0) {
        ...
        // Only fire popup if this is genuinely a NEW message we haven't seen
        if (latestId !== seenMsgIdRef.current) {
          seenMsgIdRef.current = latestId;
          // Persist so refresh doesn't re-show old messages
          try { localStorage.setItem(`seenMsg_${MY_ID}`, latestId); } catch (_) {}
    ...
    return () => unsub();
  }, []); // no deps — runs once, ref handles seen ID
  ...
  function takeBreak() {
    clearInterval(intervalRef.current);
    // Capture final time accurately
    if (startTimeRef.current) {
      const elapsed = Math.floor((Date.now() - startTimeRef.current) / 1000);
      const final = baseTimeRef.current + elapsed;
      baseTimeRef.current = final;
      setMyTime(final);
    }
    setRunning(false);
    setOnBreak(true);
    set(ref(db, "users/" + MY_ID), { status: "break", time: baseTimeRef.current, goal: myGoal, breakMsg: "Taking a short break ☕" });
  }

  function endSession() {
    clearInterval(intervalRef.current);
    // Capture final time accurately
    let finalTime = myTime;
    if (running && startTimeRef.current) {
      const elapsed = Math.floor((Date.now() - startTimeRef.current) / 1000);
      finalTime = baseTimeRef.current + elapsed;
    }
    if (finalTime > 5) {
      setSessions(prev => [{ duration: finalTime, goal: myGoal || "No goal", date: new Date().toISOString() }, ...prev]);
      setTotalToday(prev => prev + finalTime);
    }
    setRunning(false);
    setOnBreak(false);
    setMyTime(0);
  ...
  function saveGoal(g) {
    ...
    set(ref(db, "users/" + MY_ID), { status: myStatusRef.current, time: myTime, goal: g, breakMsg: "" });
  }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { background: #08030f; font-family: 'DM Sans', sans-serif; overscroll-behavior: none; }
        ...
        .card-hover:hover { background: rgba(255,255,255,0.055) !important; }
        @media (max-width: 480px) {
          .timer-grid { grid-template-columns: 1fr 1fr !important; gap: 8px !important; }
          .timer-time { font-size: 24px !important; }
          .main-pad   { padding: 1.25rem 1rem !important; }
          .tab-label  { font-size: 11px !important; }
          .love-grid  { grid-template-columns: repeat(3,1fr) !important; }
        }
      ...
      <LovePopup msg={popup?.text} from={popup?.from} onClose={handleClose} />
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            ...
            border: "0.5px solid rgba(244,63,94,0.12)",
            borderRadius: 28,
            padding: "1.75rem",
            color: "white",
            position: "relative",
        ...
        >
          ...
          <div style={{ display: "flex", gap: 3, marginBottom: "1.25rem", background: "rgba(255,255,255,0.03)", borderRadius: 14, padding: 3 }}>
            {[["main","Focus"],["stats","Stats"],["msgs","Messages"]].map(([v,l]) => (
              <button key={v} onClick={() => setView(v)} className="tab-label" style={{
                flex: 1,
                fontSize: 12,
                padding: "7px 0",
                borderRadius: 11,
          ...
          {view === "main" && <>
            ...
            {/* Timer cards */}
            <div className="timer-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: "1rem" }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                    <SDot status={status} />
                    <span style={{ fontSize: 11, color: "#94a3b8", flex: 1, letterSpacing: "0.05em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</span>
                    <span style={{ fontSize: 9, color: "#334155", background: "rgba(255,255,255,0.04)", borderRadius: 6, padding: "2px 5px", flexShrink: 0 }}>
                      {status === "studying" ? "focusing" : status === "break" ? "break" : "offline"}
                    </span>
                  </div>
                  <div className="timer-time" style={{ fontSize: 30, fontWeight: 500, letterSpacing: 1, fontVariantNumeric: "tabular-nums", marginBottom: 6 }}>{fmt(time)}</div>
                  {goal
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: "1rem", flexWrap: "wrap" }}>
              ...
              {running && (
                <button onClick={takeBreak} style={{ flex: "1 1 auto", minWidth: 100, padding: "11px 0", fontSize: 13, fontWeight: 400, borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.05)", color: "#94a3b8", cursor: "pointer", fontFamily: "inherit", WebkitTapHighlightColor: "transparent" }}>
                  ☕ Take a break
                </button>
              )}
              {(running || onBreak) && (
                <button onClick={endSession} style={{ flex: "1 1 auto", minWidth: 100, padding: "11px 0", fontSize: 13, fontWeight: 400, borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.05)", color: "#94a3b8", cursor: "pointer", fontFamily: "inherit", WebkitTapHighlightColor: "transparent" }}>
                  End session
                </button>
              )}
          ...
          {view === "stats" && <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: "1rem" }}>
              {[
                { l: "Today",    v: fmt(totalToday)              },
                { l: "Sessions", v: sessions.length              },
                { l: "Streak",   v: `${Math.min(sessions.length, 7)}d` },
              ].map(({ l, v }) => (
